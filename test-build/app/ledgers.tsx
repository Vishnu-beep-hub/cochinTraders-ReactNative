...
export default function LedgersScreen() {
  ...
  const fetchLedgers = async () => {
    ...
    getCompanyLedgers(selected).then((res: any) => {
      ...
      const filteredRows = rows.filter((l: any) => {
        const nm = String(l.$Name || l.Name || '').trim().toLowerCase();
        return nm && !nm.startsWith('_');
      });
      const mapped = filteredRows.map((l: any) => {
        const addrRaw = l.$Address ?? l.Address;
        const address = Array.isArray(addrRaw) ? addrRaw.filter(Boolean).join(', ') : String(addrRaw || '');
        return {
          id: String(l.$Name || l.Name || Math.random()),
          name: l.$Name || l.Name || '',
          mailingName: l.$MailingName || l.MailingName || '',
          closingBalance: Number(l.$ClosingBalance ?? l.ClosingBalance ?? 0) || 0,
          openingBalance: Number(l.$OpeningBalance ?? l.OpeningBalance ?? 0) || 0,
          parent: l.$Parent || l.Parent || 'N/A',
          primaryGroup: l.$_PrimaryGroup || l._PrimaryGroup || l.PrimaryGroup || '',
          address,
  ...
  return (
    <View style={styles.container}>
      <ErrorModal visible={!!errorCode} status={errorCode ?? undefined} onClose={() => setErrorCode(null)} onRetry={() => { setErrorCode(null); setLoading(true); getCompanyLedgers(selected as string).then((res: any) => { const rows = res && res.data ? res.data : []; const filteredRows = rows.filter((l: any) => { const nm = String(l.$Name || l.Name || '').trim().toLowerCase(); return nm && !nm.startsWith('_'); }); const mapped = filteredRows.map((l: any) => { const addrRaw = l.$Address ?? l.Address; const address = Array.isArray(addrRaw) ? addrRaw.filter(Boolean).join(', ') : String(addrRaw || ''); return { id: String(l.$Name || l.Name || Math.random()), name: l.$Name || l.Name || '', mailingName: l.$MailingName || l.MailingName || '', closingBalance: Number(l.$ClosingBalance ?? l.ClosingBalance ?? 0) || 0, openingBalance: Number(l.$OpeningBalance ?? l.OpeningBalance ?? 0) || 0, parent: l.$Parent || l.Parent || 'N/A', primaryGroup: l.$_PrimaryGroup || l._PrimaryGroup || l.PrimaryGroup || '', address, email: l.$Email || l.Email || '', phone: l.$Phone || l.Phone || '', companyName: l.CompanyName || '', }; }); setItems(mapped); }).catch(() => {}).finally(() => setLoading(false)); }} />
